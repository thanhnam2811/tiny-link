...
- [x] Direct unauthenticated navigation to `/`, `/links`, or `/health` redirects to `/login`.
- [x] Authenticated visits to `/login` redirect to `/`.

#### 6. ✅ Definition of Done (DoD) & Verification Plan

- [x] Verify redirects in browser and unit test.
- [x] `pnpm build` passes.
