...
- [x] Direct requests to `/api/proxy/links/user` with header `x-user-id: victim-123` without Auth0 session return `401 Unauthorized` or empty data.
- [x] Authenticated requests always inject the verified `session.user.sub` value.

#### 6. ✅ Definition of Done (DoD) & Verification Plan

- [x] Add integration test verifying forged `x-user-id` header rejection.
