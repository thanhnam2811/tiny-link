...
- [ ] Shortening links targeting `http://127.0.0.1:3001`, `localhost`, or `http://169.254.169.254` skips scraping safely with null metadata.
- [ ] Legitimate public URLs (`https://github.com`, `https://google.com`) fetch metadata normally.

#### 6. ✅ Definition of Done (DoD) & Verification Plan
- [ ] Add unit tests for `isPrivateIp` and `scrapeUrlMetadata`.
- [ ] `pnpm test` passes.
